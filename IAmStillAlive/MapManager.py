...
class MapManager:
            ...
            for mt in m.readlines():
                ...
                for a in range(len(tilemap_array)):
                    for b in range(len(tilemap_array[a].split(","))):
                        tile_index = int(tilemap_array[a].split(",")[b])
                        tile = Tile(
                            camera = camera,
                            image_path = f"resources/{new_tile_list[tile_index].strip().replace(".", "")}.png"
                        )
                        if new_tile_list[tile_index].strip().endswith("."):
                            tile.is_solid = True
                ...
                for a in range(len(objectmap_array)):
                    for b in range(len(objectmap_array[a].split(","))):
                        object_index = int(objectmap_array[a].split(",")[b]) - 1
                        if object_index >= 0:
                            game_object = GameObject(
                                camera = camera,
                                image_path = f"resources/{new_object_list[object_index].strip().replace(".", "")}.png"
                            )
                            if new_object_list[object_index].strip().endswith("."):
    ...
    def display(self):
        for tile in self.map_list[self.map_index].tile_list:
            tile.display()
        for game_object in self.map_list[self.map_index].object_list:
            game_object.display()